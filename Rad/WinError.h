...
template<class T>
inline T CheckLe(const T v, LPCSTR szModule, LPCSTR szContext, const SrcLocA src)
{
    if (!(v))
    {
        const DWORD e = GetLastError();
        RadLog(LOG_ASSERT, WinError::getMessage(e, szModule, szContext), src);
        SetLastError(e);
    }
    return v;
}
...
template<class T>
inline T CheckLe(const T v, LPCWSTR szModule, LPCWSTR szContext, const SrcLocW src)
{
    if (!(v))
    {
        const DWORD e = GetLastError();
...
template <class T>
inline T CheckLeVal(const T v, const T e, LPCSTR szModule, LPCSTR szContext, const SrcLocA src)
{
    if (v == e)
    {
        const DWORD e = GetLastError();
...
template <class T>
inline T CheckLeVal(const T v, const T e, LPCWSTR szModule, LPCWSTR szContext, const SrcLocW src)
{
    if (v == e)
    {
        const DWORD e = GetLastError();
...
#else
#define CHECK_LE(x) CheckLe(x, nullptr, TEXT(#x), SRC_LOC)
#define CHECK_LE_RET(x, r) if (!CheckLe(x, nullptr, TEXT(#x), SRC_LOC)) return (r); __assume(x)
#endif
#define CHECK_HR(x) CheckHr(x, nullptr, TEXT(#x), SRC_LOC)
#define CHECK_HR_RET(x, r) if (FAILED(CheckHr(x, nullptr, TEXT(#x), SRC_LOC))) return (r); __assume(x)
